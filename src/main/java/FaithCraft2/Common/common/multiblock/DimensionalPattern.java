package FaithCraft2.Common.common.multiblock;
...
public class DimensionalPattern {	
    ...
    public static HashMap<Character, Block> BlockStates = new HashMap<Character, Block>();
    ...
    public static void setPatternId(String id){
		patternId = id;
    }
            ...
            for (Row row : layer.rows) {
                ...
                for (int depth = 0; depth < row.sections.length(); depth++) {
                    ...
                    switch (flag) {
                        case AIR:
                            if (world.isAirBlock(new BlockPos(x + rowPos, y + layerPos, z + depth)))
                                world.setBlockState(new BlockPos(x + rowPos, y + layerPos, z + depth), BlockStates.get(blockState.type).getDefaultState());
                            break;
                        case IGNORE:
                        	world.setBlockState(new BlockPos(x + rowPos, y + layerPos, z + depth), BlockStates.get(blockState.type).getDefaultState());
                        	break;
    ...
    public static class BlockState implements IPatternComponent {
        ...
        public boolean isMatched(IBlockAccess world, int x, int y, int z) {
            return world.getBlockState(new BlockPos(x, y, z)).getBlock().equals(block);
        }
    }
